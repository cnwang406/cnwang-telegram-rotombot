...
def getLocation(bot, update):
    ...
    tmp = {'name': msg.chat.username, 'loc': [
...
def inlinequery(bot, update):
    ...
    if len(fields) == 4:
        ...
        hits = masks.findMasks(child=int(fields[0]), adult=int(
            fields[1]), distance=float(fields[2]), maxcount=int(fields[3]), sortKey=sortKey)
        #print (len(hits))
        if len(hits) != 0:
        ...
        if len(hits) == 0:
            ...
            ), title='🈚️🈚️🈚️ NO FOUND 🈚️🈚️🈚️', input_message_content=InputTextMessageContent(u'請重新設定搜尋條件,因為要求太嚴苛了'))]
        else:
            for idx, hit in enumerate(hits):
                ...
                logger.debug(
                    f'{result.latitude},{result.input_message_content}')
...
allUsers = Users(database=DB_DATABASE, user=DB_USER,
                 password=DB_USERPASSWORD, host=DB_HOST)
...
TITLE = f'''
{'*'*40}
    MASK QUERY BOT - rotom406 Ver{VERSION}
...
dispatcher.add_handler(MessageHandler(
    Filters.photo & (~Filters.forwarded), msgHandler))
